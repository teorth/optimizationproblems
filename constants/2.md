...
$C\_{2}$ is the **Crouzeix constant** (sometimes denoted $Q$).  It is the smallest constant $C$ such that for every $n \ge 1$, every complex matrix $A \in \mathbb{C}^{n \times n}$, and every complex polynomial $p$ one has

$$
\|p(A)\| \ \le\ C \ \max\_{z \in W(A)} |p(z)|,
$$
...
C\_{2}
=
\sup\_{n \ge 1}\ \sup\_{A \in \mathbb{C}^{n\times n}}\ \sup\_{p \not\equiv 0}
\frac{\|p(A)\|}{\max\_{z \in W(A)} |p(z)|}.
$$
...
- Crouzeix conjectured (in [C2004]) that $C\_{2}=2$.  The lower bound $2$ shows this would be sharp.
