...
- **This repository does not host the code or data supporting a claim.**  Verification scripts, certificates, formalizations, datasets and the like must live somewhere else — your own repository, Zenodo, arXiv ancillary files — and be linked from the constant page.  Pull requests here should consist of Markdown changes only.  This keeps the repository small, keeps it a record of bounds rather than an archive of artifacts, and leaves each contributor in control of, and responsible for, their own material.  Prefer a link that carries its own permanence: an archived DOI is better than a bare repository, and a hash or pinned commit recorded in the Comments column is better still.
